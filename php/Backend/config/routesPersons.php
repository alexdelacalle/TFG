...
return function (App $app) {
    ...
    // GET /persons/{personId}/{elementType}
    $app->get(
        $_ENV['RUTA_API'] . PersonQueryController::PATH_PERSONS . $REGEX_PERSON_ID . '/{elementType:products|entities}',
        PersonRelationsController::class . ':getElementByType'
    )->setName('readPersonElementsByType');
    ...
    // PUT /persons/{personId}/{elementType}/{operationType}/{elementId}
    $app->put(
        $_ENV['RUTA_API'] . PersonCommandController::PATH_PERSONS
            . $REGEX_PERSON_ID . '/{elementType:entities|persons}/{operationType:add|rem}' . $REGEX_ELEMENT_ID,
            PersonRelationsController::class . ':operationElementByType'
    )->setName('tdw_persons_operation_element')
        ->add(JwtMiddleware::class);
};
