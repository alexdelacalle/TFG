...
return function (App $app) {
    ...
    // RELATIONSHIPS
    // OPTIONS /entities/{entityId}[/{params:.*}]
    $app->options(
    ...
    // GET /entities/{entityId}/{elementType}
    $app->get(
        $_ENV['RUTA_API'] . EntityQueryController::PATH_ENTITIES . $REGEX_ENTITY_ID . '/{elementType:products|persons}',
        EntityRelationsController::class . ':getElementByType'
    )->setName('readEntityElementsByType');
    ...
    $app->put(
        $_ENV['RUTA_API'] . EntityCommandController::PATH_ENTITIES
            . $REGEX_ENTITY_ID . '/{elementType:products|persons}/{operationType:add|rem}' . $REGEX_ELEMENT_ID,
        EntityRelationsController::class . ':operationElementByType'
    )->setName('tdw_entities_operation_element')
        ->add(JwtMiddleware::class);
